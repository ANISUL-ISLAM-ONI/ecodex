...
class RegisterUserMixin(object):
    ...
    def register_user(self, form):
        ...
        activation_url = 'http://localhost:8000/accounts/activate-email/' + uidb64 + '/' + token + '/'
        
        if getattr(settings, 'OSCAR_SEND_REGISTRATION_EMAIL', True):
            self.send_registration_email(user, activation_url)

        return user

    def send_registration_email(self, user, activation_url):
        extra_context = {
                'user': user,
